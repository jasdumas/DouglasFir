...
#$ -o exon-tx_$JOB_ID.out

# Specify the error file
#$ -e exon-tx_$JOB_ID.err

#$ -cwd                   
#start=$(date + %s.%N)
# Run the program
echo TexasWalnut.500.scaffolds.fasta$SGE_TASK_ID.fa

cd /archive/Walnut_genomes/texas

exonerate --model protein2genome --query /archive/Walnut_genomes/spcombo.tfa --target /archive/Walnut_genomes/texas/TexasWalnut.500.scaffolds.fasta$SGE_TASK_ID.fa -n 1 --percent 70 --score 100 --minintron 20 --maxintron 800000 --showalignment no --showtargetgff yes --geneseed 250 --forcegtag --hspfilter 100 --showvulgar yes > walnut_tx.$SGE_TASK_ID.gff
